...
export async function runAgentTurn({ text, images = [], channel, convoKey, replyTs }) {
  ...
  if (imageGen) {
    ...
    if (imageGen.mode === "full") {
      // Lock the exact content so the AI renders the REAL menu and omits nothing.
      const featLine = current.featured?.name
        ? `${current.featured.name}${current.featured.price ? ` — $${current.featured.price}` : ""}`
        : "(none)";
      const entreeLines = current.entrees.map(
...
Create a PORTRAIT restaurant daily-specials chalkboard. It MUST contain EVERY line below — do not omit, add, rename, reorder, or re-price anything. Spell everything exactly; right-align prices.
...
SIDES: ${current.sides.join(", ") || "(none)"}
SOUPS: ${current.soups.join(", ") || "(none)"}

Before finishing, COUNT the entrées in your image — there must be exactly ${entreeLines.length}, plus the featured title. ${refs.length ? "Match the provided reference image's style: wood frame, colored chalk lettering, decorative flourishes, dotted dividers." : "Style: realistic wood-framed black chalkboard, colored chalk lettering, decorative flourishes, dotted dividers."}`;
