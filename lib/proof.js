...
FRAMING (important): a flat rectangular wooden picture frame directly around a SINGLE black chalkboard that completely fills the inside of the frame. NO easel, NO stand or tripod, NO inner border/box/outline/extra margin around the text. CROP TIGHT: the wooden frame fills the ENTIRE image — its outer edges run right to all four borders of the picture, like a close-up straight-on photo of just the framed board, with at most a thin sliver of background showing. Any background that does show must be PLAIN, SOLID, light cream/off-white — absolutely NO brick wall, NO wood planks, NO textured or patterned wall, NO room or restaurant scene, NO props. ${refs.length ? "Match the attached image's visual STYLE only (hand-drawn colored chalk, flourishes, dotted dividers, botanical sprigs in the bottom corners) — IGNORE any text/items in it; the menu is exactly the list above." : "Hand-lettered chalk: a colored title, cream item names with a few tasteful color accents, small decorative flourishes, dotted divider lines, and simple botanical sprigs in the bottom corners."}`;

  const { data } = await generateImage({ prompt, references: refs });
  // Auto-crop the plain cream border down to the wooden frame (like the reference).
  let out = data;
  try {
    out = await sharp(data).trim({ threshold: 18 }).png().toBuffer();
  } catch (e) {
    console.error("crop failed, using uncropped image:", e.message);
  }
  return await uploadImage(out, "image/png");
}
