...
Show each item's name on the left and its price ONCE on the right — never repeat a price. Before finishing, COUNT the entrées in your image — there must be exactly ${entreeLines.length}, plus the featured title. ${refs.length ? "Use the attached image ONLY as a visual STYLE reference (wood frame, colored chalk lettering, decorative flourishes, dotted dividers) — IGNORE any text or menu items shown in it; the menu content is exactly the list above." : "Style: realistic wood-framed black chalkboard, colored chalk lettering, decorative flourishes, dotted dividers."}`;
...
export async function generateProof({ instruction, channel, continueFromDraft = false }) {
  ...
  // Render as the hand-drawn AI board (same as the conversational path) so the
  // slash command AND reject->feedback revisions never downgrade to the flat template.
  const image =
    geminiConfigured() && blobConfigured() ? await generateFullBoardImage(menu) : null;
...
export async function runAgentTurn({ text, images = [], channel, convoKey, replyTs }) {
  ...
  if (boardUpdate) {
    ...
    // ALWAYS render as the hand-drawn AI chalkboard (the only mode). Falls back to
    // the template only if AI image generation isn't configured at all.
    const useAI = geminiConfigured() && blobConfigured();
    let draft;
    if (useAI) {
