...
- update_board: clean template board with guaranteed-perfect text. Use only for a small one-line correction to an existing board, or when the owner explicitly says they want exact/guaranteed text. Return the COMPLETE board, carrying over anything unchanged.
- generate_background: generate a custom AI ART background; the system overlays the exact menu text on top (prices/spelling stay perfect). Use for richer/custom/seasonal looks or to match a reference photo's vibe. Always provide textColor + accentColor that contrast with your described background.
- generate_full_image: generate the WHOLE board as one AI image including the text — this is the ONLY way to get a hand-drawn, multi-color, decorated chalkboard like the owner's reference photo (colored chalk lettering, flourishes, laurels, etc.). Use it whenever they want the board to look like their reference / "our usual board" / a real hand-lettered chalkboard. The system automatically supplies the saved house-style reference image and locks in the exact menu, so a short prompt is fine. Text accuracy isn't 100% guaranteed — remind them to glance at the proof before approving.
DEFAULT: for almost any request to create, rebuild, or restyle the board, use generate_full_image — the owner prefers the hand-drawn AI chalkboard look. Remind them to glance at the proof for typos. (generate_background = AI backdrop with clean overlaid text; update_board = guaranteed-exact text for a quick one-line fix.)
...
- To make the board LOOK like the owner's hand-drawn chalkboard reference (colored chalk, flourishes, laurels), use generate_full_image — it reuses the saved reference image. Use template themes only for simpler/clean looks.
...
export async function aiAgent({ history = [], userText = "", images = [], currentBoard, dateStr, holiday, prefs = "" }) {
  ...
  for (const c of data.content || []) {
    ...
    if (c.type === "tool_use") {
      if (c.name === "update_board") boardUpdate = normalizeSpecial(c.input);
