...
export async function generateImage({ prompt, references = [] }) {
  ...
  const requestBody = JSON.stringify({
    ...
    generationConfig: {
      ...
      imageConfig: { aspectRatio: "3:4" }, // tall portrait so the framed board fills the image
    },
  });

  // Retry transient failures (rate limits / 5xx / empty responses) before giving up.
  let lastErr;
